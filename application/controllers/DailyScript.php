...
class DailyScript extends CI_Controller {
	function __construct()  
        { 
            parent::__construct();
			$this->load->model("WeeklyScript_model","DailyScript"); 
	...
	public function clearWeeklyScore(){
		$userScore_arr = $this->DailyScript->clear();
	}
	...
	public function calcReward(){   
		//检查设备登录的数据库信息 
		$userScore_arr = $this->DailyScript->getWeeklyScoreInfo();
		if($userScore_arr != null)
		{
			...
			foreach($userScore_arr as $userid => $score)
			{
				...
				if(count($friendInfo["friends"]) < 10){//好友数目超过10个才算
					if(count($addMail_arr)){
						$this->Player->addMail($userid,$addMail_arr);
					}
					continue;
				...
				foreach($friendInfo["friends"] as $key_id => $value_id){
